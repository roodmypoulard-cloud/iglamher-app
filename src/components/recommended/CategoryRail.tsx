...
const AllIcon = (p: IconProps) => (
  <svg viewBox="0 0 24 24" width={24} height={24} {...stroke} className={p.className} aria-hidden>
...
export function CategoryRail({ categories }: { categories: Array<{ slug: CategorySlug; name: string }> }) {
  ...
  return (
    <div role="tablist" aria-label="Service category" className="scrollbar-none -mx-5 flex gap-3 overflow-x-auto px-5 py-1">
      {tiles.map((t) => {
        ...
        return (
          <Link
            ...
            scroll={false}
            className="flex flex-none flex-col items-center gap-1.5 active:scale-95"
          >
            {/* Circular icon chip with the label beneath — matches the design mock. */}
            <span
              className={cn(
                "grid h-[64px] w-[64px] place-items-center rounded-full border transition-[border-color,box-shadow] duration-200",
                active
                  ? "fab-gold border-transparent shadow-[0_8px_22px_rgba(215,160,143,0.4)]"
                  : "border-rose/20 bg-surface hover:border-rose/45",
              )}
            >
              <Icon className={active ? "text-[#2A1712]" : "text-rose"} />
            </span>
            <span className={cn("text-[11px] font-semibold leading-none", active ? "font-bold text-rose" : "text-ink-secondary")}>{t.name}</span>
          </Link>
        );
      })}
    </div>
