...
export function RecommendedProCard({
  ...
  return (
    <article className="overflow-hidden rounded-[18px] border border-rose/[0.16] bg-surface shadow-[inset_0_1px_0_rgba(255,248,244,0.05),0_8px_26px_rgba(0,0,0,0.3)]">
      <div className="flex">
        {/* Portfolio image — the card's hero */}
        <Link href={profileHref} className="relative w-[34%] min-w-[104px] max-w-[136px] flex-none self-stretch" aria-label={`View ${pro.displayName}'s profile`}>
          <SmartImage
            src={cover}
        ...
        <div className="min-w-0 flex-1 p-3">
          <h3 className="flex items-center gap-1.5 font-display text-[15px] font-bold leading-tight text-ink">
          ...
          <p className="mt-0.5 truncate text-[11.5px] text-ink-secondary">{pro.primarySpecialty}</p>
          <p className="mt-0.5 truncate text-[10.5px] text-ink-muted">
            {pro.city}
            {pro.distanceMi != null && ` · ${formatDistance(pro.distanceMi)}`}
          </p>

          <p className="mt-1.5 flex items-center gap-1.5 text-[11.5px] text-ink-secondary">
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-1">
            {nextOpening && (
              <span className="rounded-full bg-success/[0.12] px-2 py-0.5 text-[9.5px] font-bold text-success">
                Next: {nextOpening}
              </span>
            )}
            {pro.isVerified && (
              <span className="rounded-full border border-rose/30 px-2 py-0.5 text-[9.5px] font-semibold text-rose">
                Verified
              </span>
            )}
            {pro.locationType !== "in_salon" && (
              <span className="rounded-full border border-border px-2 py-0.5 text-[9.5px] font-semibold text-ink-muted">
          ...
          <div className="mt-2.5 flex items-center justify-end gap-2">
            <Link
              href={profileHref}
              className="flex min-h-[36px] items-center whitespace-nowrap rounded-full border border-border px-3 text-[11.5px] font-semibold text-ink-secondary transition-colors hover:border-rose/50 hover:text-ink active:scale-[0.98]"
            >
              View Profile
            </Link>
            <Link
              href={`/book/${pro.slug}`}
              className="flex min-h-[36px] items-center whitespace-nowrap rounded-full rose-gradient px-3.5 text-[11.5px] font-bold text-[#2A1712] shadow-[0_6px_16px_rgba(215,160,143,0.3)] transition-transform active:scale-[0.98]"
            >
              Book Now
            </Link>
          </div>
