...
import {
  ...
  homeStudioNeedsReview, type ServiceLocationKey, type LocationCompliance, type YesNo,
...
export function ServiceLocationStep({
  ...
  const showHome = locations.has("home_studio") || locations.has("multiple");
  ...
  async function save() {
    ...
    const [locRes, agrRes] = await Promise.all([
      saveServiceLocationsAction({ locations: [...locations], compliance, hideExactPin: hidePin }),
    ...
    <div className="space-y-5">
      ...
      {showHome && (
        <div className="rounded-[16px] border border-rose/25 bg-surface p-4">
          ...
          <p className="mt-0.5 text-[11.5px] text-ink-muted">A few questions so we can review your home-based location.</p>
          <div className="mt-3 space-y-2.5">
            {HOME_STUDIO_QUESTIONS.map((q) => (
              <div key={q.key} className="flex items-start justify-between gap-3">
                <span className="min-w-0 flex-1 text-[12px] leading-snug text-ink-secondary">
          ...
          <label className="mt-3 flex cursor-pointer items-center gap-2 text-[12px] text-ink-secondary">
            <input type="checkbox" checked={hidePin} onChange={(e) => setHidePin(e.target.checked)} className="h-4 w-4 accent-[var(--color-rose)]" />
