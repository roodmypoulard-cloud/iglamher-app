...
export default function ProfessionalProfileError({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => {
    console.error(error);
  }, [error]);
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-[440px] flex-col items-center justify-center px-6 text-center">
      <h1 className="font-display text-xl font-bold text-ink">We couldn&apos;t load this profile.</h1>
      ...
      <Link href="/discover" className="mt-3 text-[13px] font-semibold text-ink underline-offset-4 hover:underline">
