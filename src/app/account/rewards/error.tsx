...
export default function RewardsError({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-[440px] flex-col items-center justify-center px-6 text-center">
      <h1 className="font-display text-xl font-bold text-ink">We couldn&apos;t load your rewards.</h1>
      <p className="mt-1.5 text-[13px] text-ink-secondary">Your points are safe — this is just a display issue.</p>
      <button
        type="button"
        onClick={reset}
